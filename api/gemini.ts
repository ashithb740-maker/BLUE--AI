...
export default async function handler(req: RequestWithBody, res: ResponseLike) {
  ...
  const quotaResponse = await supabase("/rest/v1/rpc/reserve_ai_tokens", bearer, {
    method: "POST",
  ...
  if (!quotaResponse.ok || !quota?.allowed) {
    return res.status(429).json({ error: "You have reached today's free BLUE limit. Upgrade to BLUE Pro for ₹11/year and keep chatting without the daily limit.", code: "DAILY_LIMIT", remainingTokens: quota?.remaining_tokens ?? 0 });
  }
  ...
  try {
    ...
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent", {
      ...
      body: JSON.stringify({ contents: safeHistory, systemInstruction: { parts: [{ text: BLUE_SYSTEM_PROMPT }] }, generationConfig: { temperature: 0.7, maxOutputTokens: 4096 } }),
    });
    const data = await response.json();
    if (!response.ok) return res.status(response.status).json({ error: "The AI service could not complete that request." });
    const text = (data?.candidates?.[0]?.content?.parts ?? []).map((part: any) => part?.text ?? "").join("").trim();
    if (!text) return res.status(502).json({ error: "The AI service returned an empty response." });

    const actualTokens = Number(data?.usageMetadata?.totalTokenCount || estimatedTokens);
    const delta = estimatedTokens - actualTokens;
    if (delta) await supabase("/rest/v1/rpc/adjust_ai_tokens", bearer, { method: "POST", body: JSON.stringify({ p_delta: -delta }) }).catch(() => undefined);
    ...
    if (!activeConversationId) {
      ...
      const created = await supabase("/rest/v1/conversations", bearer, { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ user_id: user.id, title }) });
      if (created.ok) { const rows = await created.json(); activeConversationId = rows?.[0]?.id || null; }
    }
    if (activeConversationId) {
      await supabase("/rest/v1/messages", bearer, { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify([{ conversation_id: activeConversationId, user_id: user.id, role: "user", content: message }, { conversation_id: activeConversationId, user_id: user.id, role: "model", content: text }]) });
