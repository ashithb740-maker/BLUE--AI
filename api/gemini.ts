...
export default async function handler(req: RequestWithBody, res: ResponseLike) {
  ...
  const estimatedTokens = Math.min(
    1500,
    Math.max(250, Math.ceil((message.length + JSON.stringify(history.slice(-6)).length) / 4) + 700),
  );
  ...
  try {
    ...
    const safeHistory = buildGeminiHistory(history, message, timeZone);
    ...
    const returnedId = activeConversationId != null ? String(activeConversationId) : conversationId;

    return res.status(200).json({
      text,
      conversationId: returnedId,
      plan: isOwner ? "pro" : (quota?.plan || "free"),
      remainingTokens: isOwner
        ? null
        : Math.max(0, Number(quota?.remaining_tokens ?? 0) - Math.max(0, actualTokens - estimatedTokens)),
    });
  } catch (error) {
    if (!isOwner) {
      await supabase("/rest/v1/rpc/adjust_ai_tokens", bearer, {
        method: "POST",
        body: JSON.stringify({ p_delta: -estimatedTokens }),
      }).catch(() => {});
    }
