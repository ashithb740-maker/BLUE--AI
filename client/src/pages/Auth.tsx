...
import { getSession, setSession, signIn, signUp, resendConfirmation, refreshSession } from "@/lib/supabase";
...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (current) {
        ...
        if (!dead && (refreshed || getSession())) { goChat(); return; }
      }
      try {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError(""); setMessage(""); setShowResend(false); setLoading(true);
    try {
      if (mode === "signin") {
        ...
        if (result.session) {
          ...
          setMessage(`We've created your BLUE AI account. Please check ${email.trim()} for your confirmation email, then return here to sign in.`);
          setShowResend(true);
        }
      }
    } catch (e) {
      ...
      setError(text);
      if (/not confirmed|confirm/i.test(text)) setShowResend(true);
  ...
  const resend = async () => {
    ...
    try {
      ...
      setMessage(`A fresh BLUE AI confirmation email was sent to ${email.trim()}. Check your inbox and spam folder.`);
    } catch (e) { setError(e instanceof Error ? e.message : "We couldn't resend the confirmation email."); }
    finally { setLoading(false); }
  };
