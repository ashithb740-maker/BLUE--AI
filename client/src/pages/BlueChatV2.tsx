...
import { ArrowUp, Bot, Check, Copy, Menu, Plus, Sparkles, User, X, LogOut, MoreVertical, Pin, PinOff, Trash2 } from "lucide-react";
import { Link, useLocation } from "wouter";
import { Streamdown } from "streamdown";
import { Button } from "@/components/ui/button";
import { dbFetch, getSession, refreshSession, setSession, signOut } from "@/lib/supabase";
...
export default function BlueChatV2() {
  ...
  const sortChats = (items: Conversation[]) => [...items].sort((a, b) => {
    if (!!a.pinned !== !!b.pinned) return a.pinned ? -1 : 1;
    return +new Date(b.updated_at) - +new Date(a.updated_at);
  });

  const loadChats = async () => {
    const rows = await dbFetch("/conversations?select=id,title,updated_at,pinned&order=pinned.desc,updated_at.desc&limit=200");
  ...
  useEffect(() => {
    ...
    const keepSignedIn = window.setInterval(async () => {
      if (getSession()) await refreshSession();
    }, 45 * 60 * 1000);
    const onVisible = async () => { if (document.visibilityState === "visible" && getSession()) await refreshSession(); };
    document.addEventListener("visibilitychange", onVisible);
    return () => { dead = true; window.clearInterval(keepSignedIn); document.removeEventListener("visibilitychange", onVisible); };
  }, [navigate]);

  useEffect(() => { endRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, loading]);

  const loadChat = async (chat: Conversation) => {
    setSidebar(false); setMenuId(null); setError(""); setLoading(true); setConversationId(chat.id);
    try {
      const rows = await dbFetch(`/messages?select=role,content,created_at&conversation_id=eq.${encodeURIComponent(chat.id)}&order=created_at.asc&limit=1000`);
  ...
  const newChat = () => { setMessages([]); setInput(""); setConversationId(null); setError(""); setMenuId(null); setSidebar(false); window.history.replaceState({}, "", "/chat"); };

  const togglePin = async (chat: Conversation) => {
    setMenuId(null);
    try {
      await dbFetch(`/conversations?id=eq.${encodeURIComponent(chat.id)}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ pinned: !chat.pinned }) });
      setChats(prev => sortChats(prev.map(x => x.id === chat.id ? { ...x, pinned: !x.pinned } : x)));
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to update this chat."); }
  };
  ...
  const deleteChat = async (chat: Conversation) => {
    ...
    try {
      await dbFetch(`/conversations?id=eq.${encodeURIComponent(chat.id)}`, { method: "DELETE" });
      setChats(prev => prev.filter(x => x.id !== chat.id));
      if (conversationId === chat.id) newChat();
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete this chat."); }
  };

  const send = async (value = input) => {
    const text = value.trim(); if (!text || loading) return;
    let session = getSession(); if (!session) { navigate("/auth"); return; }
    const before = messages;
    setMessages([...before, { role: "user", text }]); setInput(""); setLoading(true); setError("");
    try {
      const callApi = async (accessToken: string) => fetch("/api/gemini", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ message: text, history: before.slice(-20), timeZone: timezone(), conversationId }) });
      let response = await callApi(session.access_token);
      if (response.status === 401) {
        session = await refreshSession();
        if (!session) { setSession(null); navigate("/auth"); return; }
        response = await callApi(session.access_token);
      }
      const data = await response.json().catch(() => ({}));
      if (!response.ok) { setMessages(before); setError(data?.error || "BLUE could not answer right now. Please try again."); return; }
      ...
      if (id) {
        await loadChats().catch(() => {
          const fallback: Conversation = { id, title: text.slice(0, 70), updated_at: new Date().toISOString(), pinned: false };
          setChats(prev => sortChats([fallback, ...prev.filter(x => x.id !== id)]));
        });
      }
  ...
  return <div className="flex h-dvh overflow-hidden bg-[#08090d] text-white">
    ...
    <aside className={`${sidebar ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 flex w-[300px] flex-col border-r border-white/[.08] bg-[#0b0d12]/95 p-3 backdrop-blur-xl transition-transform md:relative md:translate-x-0`}>
      ...
      <div className="mt-7 flex items-center justify-between px-2"><span className="text-[10px] font-semibold uppercase tracking-[.18em] text-white/30">Saved chats</span><span className="text-[10px] text-white/20">{chats.length}</span></div>
      <div className="mt-2 flex-1 overflow-y-auto space-y-1 pr-1 blue-scroll">
        {chats.length ? chats.map(chat => <div key={chat.id} className={`group relative rounded-xl transition ${chat.id === conversationId ? "bg-white/[.08]" : "hover:bg-white/[.05]"}`}>
          <button onClick={() => loadChat(chat)} className={`w-full rounded-xl px-3 py-2.5 pr-10 text-left text-[13px] ${chat.id === conversationId ? "text-white" : "text-white/55 group-hover:text-white/85"}`}>
            <span className="flex items-center gap-2"><span className="truncate">{chat.title}</span>{chat.pinned && <Pin className="size-3 shrink-0 text-cyan-300" />}</span>
          </button>
          <button aria-label="Chat options" onClick={() => setMenuId(menuId === chat.id ? null : chat.id)} className="absolute right-1.5 top-1.5 rounded-lg p-1.5 text-white/25 opacity-0 transition group-hover:opacity-100 hover:bg-white/10 hover:text-white"><MoreVertical className="size-4" /></button>
          {menuId === chat.id && <div className="absolute right-2 top-10 z-50 w-36 rounded-xl border border-white/10 bg-[#151820] p-1 shadow-2xl">
            <button onClick={() => togglePin(chat)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs text-white/70 hover:bg-white/[.07]">{chat.pinned ? <PinOff className="size-3.5" /> : <Pin className="size-3.5" />}{chat.pinned ? "Unpin" : "Pin chat"}</button>
            <button onClick={() => deleteChat(chat)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-xs text-red-300/80 hover:bg-red-400/10"><Trash2 className="size-3.5" /> Delete chat</button>
          </div>}
        </div>) : <p className="px-3 py-3 text-xs text-white/25">Your conversations will remain here.</p>}
      </div>
      <div className="rounded-2xl border border-white/[.08] bg-white/[.025] p-3"><div className="flex items-center gap-2 text-xs font-medium"><Sparkles className="size-3.5 text-cyan-300" /> BLUE AI</div><p className="mt-2 text-[10px] text-white/30">Your chats are saved to your account.</p></div>
    </aside>
