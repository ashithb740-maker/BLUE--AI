...
export default function Home() {
  ...
  const goToChat = (value?: string) => {
    ...
    if (text) window.sessionStorage.setItem("nova-initial-prompt", text);
    window.location.href = "/chat";
  };

  return (
    <main className={`${theme === "light" ? "blue-light-page" : ""} min-h-screen overflow-hidden bg-[#08090d] text-white`}>
      <div className="pointer-events-none fixed inset-0 blue-grid opacity-40" />
      <div className="pointer-events-none fixed -left-48 top-[-16rem] h-[42rem] w-[42rem] rounded-full bg-blue-600/20 blur-[120px]" />
      ...
      <nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-12">
        <Link href="/" className="flex items-center gap-3" aria-label="BLUE AI home">
          <span className="blue-mark"><Sparkles className="size-4" /></span>
          <span className="text-lg font-semibold tracking-[-0.04em]">BLUE <span className="text-white/45">AI</span></span>
        </Link>
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-white/[.04] p-1">
            <button onClick={() => setTheme("light")} className={`flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] ${theme === "light" ? "bg-white text-slate-900" : "text-white/55 hover:text-white"}`}><Sun className="size-3.5" />Light</button>
            <button onClick={() => setTheme("dark")} className={`flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] ${theme === "dark" ? "bg-white/10 text-white" : "text-white/55 hover:text-white"}`}><Moon className="size-3.5" />Dark</button>
          </div>
          <Button onClick={() => goToChat()} className="rounded-full bg-blue-500 px-5 hover:bg-blue-400">Open BLUE <ArrowRight className="size-4" /></Button>
        </div>
      ...
      {mobileMenu && <div className="relative z-20 mx-5 rounded-2xl border border-white/10 bg-[#11131a]/95 p-4 shadow-2xl sm:hidden"><div className="flex flex-col gap-4 text-sm text-white/70"><a href="#capabilities" onClick={() => setMobileMenu(false)}>Capabilities</a><a href="#how-it-works" onClick={() => setMobileMenu(false)}>How it works</a><button className="text-left" onClick={() => goToChat()}>Open BLUE</button></div></div>}
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl items-center gap-16 px-5 pb-24 pt-16 sm:px-8 md:pt-24 lg:grid-cols-[1.08fr_.92fr] lg:px-12 lg:pb-36 lg:pt-28">
        <div>
          ...
          <div className="mt-9 max-w-2xl rounded-[1.5rem] border border-white/10 bg-white/[.055] p-2 shadow-2xl shadow-blue-950/20 backdrop-blur-xl">
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); goToChat(); } }} rows={2} placeholder="Ask BLUE anything..." className="w-full resize-none bg-transparent px-4 py-3 text-base text-white outline-none placeholder:text-white/28" />
            <div className="flex items-center justify-between px-2 pb-1"><div className="flex items-center gap-1.5 text-xs text-white/35"><button className="rounded-lg p-2 hover:bg-white/10" aria-label="Add attachment"><FileSearch className="size-4" /></button><button className="rounded-lg p-2 hover:bg-white/10" aria-label="Voice input"><Mic className="size-4" /></button><span className="ml-1 hidden sm:inline">Press Enter to start</span></div><Button onClick={() => goToChat()} disabled={!prompt.trim()} className="rounded-xl bg-white px-4 text-[#0b0c10] hover:bg-cyan-100 disabled:bg-white/10 disabled:text-white/30">Begin <ArrowRight className="size-4" /></Button></div>
          </div>

          <div className="mt-5 flex flex-wrap gap-2">{examples.map(item => <button key={item} onClick={() => goToChat(item)} className="rounded-full border border-white/10 bg-white/[.025] px-3.5 py-2 text-xs text-white/48 transition-all hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/[.07] hover:text-white/80">{item}</button>)}</div>
        </div>

        <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} className="relative mx-auto w-full max-w-[29rem] lg:ml-auto">
          <div className="absolute inset-10 rounded-full bg-blue-500/25 blur-[90px]" />
          <div className="relative rounded-[2rem] border border-white/10 bg-white/[.045] p-3 shadow-[0_30px_100px_rgba(53,26,112,.35)] backdrop-blur-xl">
            <div className="rounded-[1.4rem] border border-white/10 bg-[#0d0f15] p-5 sm:p-7">
              <div className="mb-10 flex items-center justify-between"><div className="flex items-center gap-2 text-sm text-white/70"><span className="blue-mark size-7"><Sparkles className="size-3.5" /></span> BLUE <span className="text-white/30">/</span> New chat</div><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2.5 py-1 text-[10px] text-emerald-300">ONLINE</span></div>
              <div className="flex flex-col items-center text-center"><div className="blue-orb mb-7"><div className="blue-orb-core"><Sparkles className="size-9 text-white" /></div></div><p className="text-2xl font-medium tracking-[-.04em]">What will we make?</p><p className="mt-2 max-w-xs text-sm leading-6 text-white/40">A calm, capable space for your best questions and ideas.</p></div>
              <div className="mt-12 space-y-2.5">{["Summarize the latest research", "Help me think through a problem", "Turn an idea into a prototype"].map((item, i) => <button key={item} onClick={() => goToChat(item)} className="flex w-full items-center gap-3 rounded-xl border border-white/[.07] bg-white/[.025] px-3.5 py-3 text-left text-xs text-white/48 hover:bg-white/[.06]"><span className="text-white/20">0{i + 1}</span>{item}<ArrowRight className="ml-auto size-3.5 text-white/20" /></button>)}</div>
            </div>
          </div>
        </motion.div>
      </section>
