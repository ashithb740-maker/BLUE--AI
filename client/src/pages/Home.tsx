  ...
  return (
    <main className={`${theme === "light" ? "blue-light-page" : ""} min-h-screen overflow-hidden bg-[#08090d] text-white`}>
      ...
      <nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-12">
        ...
        <div className="hidden items-center gap-2 sm:flex"><div className="flex items-center gap-1 rounded-xl border border-white/10 bg-white/[.04] p-1" aria-label="Theme options"><button onClick={() => setTheme("light")} className={`flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] ${theme === "light" ? "bg-white text-slate-900" : "text-white/55 hover:text-white"}`} aria-label="Use light theme"><Sun className="size-3.5" />Light</button><button onClick={() => setTheme("dark")} className={`flex items-center gap-1 rounded-lg px-2 py-1.5 text-[10px] ${theme === "dark" ? "bg-white/10 text-white" : "text-white/55 hover:text-white"}`} aria-label="Use dark theme"><Moon className="size-3.5" />Dark</button></div>
          ...
          {!user && <Button className="rounded-full bg-blue-500 px-5 hover:bg-blue-400" onClick={() => goToChat()}>Start chatting <ArrowRight className="size-4" /></Button>}
        </div>
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl items-center gap-16 px-5 pb-24 pt-16 sm:px-8 md:pt-24 lg:grid-cols-[1.08fr_.92fr] lg:px-12 lg:pb-36 lg:pt-28">
        <div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, delay: .05 }} className="max-w-3xl text-5xl font-semibold leading-[1.02] tracking-[-0.065em] text-white sm:text-7xl">Ask anything.<br /><span className="blue-gradient-text">Build anything.</span></motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55, delay: .18 }} className="mt-9 max-w-2xl rounded-[1.5rem] border border-white/10 bg-white/[.055] p-2 shadow-2xl shadow-blue-950/20 backdrop-blur-xl">
      ...
      <footer className="relative z-10 mx-auto flex max-w-7xl flex-col gap-5 border-t border-white/[.07] px-5 py-8 text-xs text-white/35 sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12"><div className="flex items-center gap-2"><span className="blue-mark size-6"><Sparkles className="size-3" /></span><span>BLUE AI</span></div><div className="flex gap-5"><a href="#privacy" className="hover:text-white">Privacy</a><a href="#privacy" className="hover:text-white">Terms</a><a href="mailto:hello@nova.example" className="hover:text-white">Contact</a></div><span>© 2026 BLUE AI</span></footer>
    </main>
  );
}
