  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#08090d] text-white">
      ...
      <nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-12">
        <Link href="/" className="flex items-center gap-3" aria-label="NOVA AI home">
          <span className="nova-mark"><Sparkles className="size-4" /></span>
          <span className="tracking-[-0.04em] text-lg font-semibold">NOVA <span className="text-white/45">AI</span></span>
        </Link>
        ...
        <div className="hidden items-center gap-3 sm:flex">
          {user ? <Link href="/chat"><Button className="rounded-full bg-white px-5 text-[#0b0c10] hover:bg-white/90">Open NOVA <ArrowRight className="size-4" /></Button></Link> : <Button variant="ghost" className="rounded-full text-white/70 hover:bg-white/10 hover:text-white" onClick={startLogin}>Sign in</Button>}
      ...
      {mobileMenu && <div className="relative z-20 mx-5 rounded-2xl border border-white/10 bg-[#11131a]/95 p-4 shadow-2xl sm:hidden"><div className="flex flex-col gap-4 text-sm text-white/70"><a href="#capabilities" onClick={() => setMobileMenu(false)}>Capabilities</a><a href="#how-it-works" onClick={() => setMobileMenu(false)}>How it works</a><button className="text-left" onClick={user ? () => window.location.href = "/chat" : startLogin}>{user ? "Open NOVA" : "Sign in"}</button></div></div>}
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl items-center gap-16 px-5 pb-24 pt-16 sm:px-8 md:pt-24 lg:grid-cols-[1.08fr_.92fr] lg:px-12 lg:pb-36 lg:pt-28">
        <div>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55, delay: .18 }} className="mt-9 max-w-2xl rounded-[1.5rem] border border-white/10 bg-white/[.055] p-2 shadow-2xl shadow-violet-950/20 backdrop-blur-xl">
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); goToChat(); } }} rows={2} placeholder="Ask Nova anything..." className="w-full resize-none bg-transparent px-4 py-3 text-base text-white outline-none placeholder:text-white/28" aria-label="Ask Nova anything" />
