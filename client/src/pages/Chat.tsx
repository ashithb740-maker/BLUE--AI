...
import { Archive, ArrowDown, Check, ChevronDown, Clipboard, Copy, FileText, Image as ImageIcon, LogOut, Menu, MessageSquare, Mic, MoreHorizontal, Paperclip, PanelLeftClose, PanelLeftOpen, Plus, Search, Send, Settings, Sparkles, Square, Sun, Trash2, UserRound, Volume2, X, Zap } from "lucide-react";
...
function NovaOrb({ small = false, active = false }: { small?: boolean; active?: boolean }) {
  return <div className={`nova-orb ${small ? "scale-[.58]" : ""} ${active ? "nova-orb-active" : ""}`}><div className="nova-orb-core"><Sparkles className="size-7 text-white" /></div></div>;
}
...
export default function Chat() {
  ...
  const submit = (value?: string) => {
    ...
    sendMutation.mutate({ conversationId: selectedId ?? undefined, content: optimistic.content, mode, model, attachments }, {
      ...
      onError: error => { setIsGenerating(false); setMessages(prev => prev.filter(message => !message.pending)); toast.error(error.message || "NOVA could not respond"); },
    });
  };
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-[#08090d] text-white"><NovaOrb active /></div>;
  if (!isAuthenticated) return <div className="flex min-h-screen items-center justify-center bg-[#08090d] px-5 text-white"><div className="text-center"><NovaOrb /><h1 className="mt-3 text-2xl font-semibold">Sign in to open NOVA</h1><p className="mt-2 text-sm text-white/45">Your conversations are private and saved to your account.</p><Button onClick={startLogin} className="mt-7 rounded-full bg-white px-6 text-[#0b0c10]">Continue with NOVA <ArrowDown className="size-4 rotate-[-45deg]" /></Button><Link href="/" className="mt-5 block text-sm text-white/40 hover:text-white">Back to home</Link></div></div>;
  ...
  return <div className="flex h-dvh overflow-hidden bg-[#08090d] text-white">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[286px] flex-col border-r border-white/[.08] bg-[#0c0e13] transition-transform duration-200 lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "lg:w-[76px]" : ""}`}>
      <div className={`flex h-16 items-center ${collapsed ? "justify-center" : "justify-between px-4"}`}><Link href="/" className="flex items-center gap-2.5"><span className="nova-mark size-8"><Sparkles className="size-4" /></span>{!collapsed && <span className="text-sm font-semibold tracking-[-.03em]">NOVA <span className="text-white/35">AI</span></span>}</Link>{!collapsed && <button className="rounded-lg p-2 text-white/35 hover:bg-white/5 hover:text-white" onClick={() => setCollapsed(true)} aria-label="Collapse sidebar"><PanelLeftClose className="size-4" /></button>}</div>
